...
class PDFPageView {
  ...
  constructor(options) {
    ...
    this.fabricLayerFactory = options.fabricLayerFactory
  ...
  async setPdfPage(pdfPage) {
    ...
    this.annotationLayer.eventBus = this.eventBus
  ...
  async _renderFabricLayer() {
    try {
      await this.fabricLayer.render()
    } catch (ex) {}
  }
  ...
  cssTransform({
    ...
    if (this.fabricLayer) {
      this.fabricLayer.cssTransform(this.viewport)
    }
  }
        ...
        return finishPaintTask(null).then(() => {
          ...
          if (this.annotationLayer) {
            this._renderAnnotationLayer().then(() => {
              if (this.fabricLayerFactory && this.canvas) {
                if (!this.fabricLayer) {
                  this.fabricLayer =
                    this.fabricLayerFactory.createFabricLayerBuilder(
                      canvasWrapper,
                      this.id - 1,
                      this.canvas,
                      this.scale,
          ...
          if (this.watermark && this.watermark.watermarkBase64) {
            ...
            watermark.onload = function () {
              ...
              const originW = (this.width * self.scale * 96.0) / 72.0
              const originH = (this.height * self.scale * 96.0) / 72.0
              let width = originW * ratio
              let height = originH * ratio
              if (originW > self.canvas.width) {
              ...
              ctx.drawImage(
                watermark,
                0.5 * (self.canvas.width - width),
                0.5 * (self.canvas.height - height),
                width,
                height
              )
              ctx.restore()
            ...
            watermark.src =
              'data:image/png;base64,' + this.watermark.watermarkBase64
          }
        })
      },
      function (reason) {
